//go:generate echo "https://atcoder.jp/contests/abc015/tasks/abc015_4"
package main
...
func Solve(ABs [][]int, N, W, K int) int {
	t := make([][][]int, N+1, N+1)
	for i := range t {
		t[i] = make([][]int, K+1, K+1)
		for j := range t[i] {
			t[i][j] = make([]int, W+1, W+1)
		}
	}
	...
	for i := 1; i <= N; i++ {
		for k := 1; k <= K; k++ {
			for w := 0; w <= W; w++ {
				A, B := ABs[i-1][0], ABs[i-1][1]
				if 0 <= w-A {
					chmax(&t[i][k][w], t[i-1][k-1][w-A]+B) // 採用するとき
				}
				chmax(&t[i][k][w], t[i-1][k][w]) // 採用しないとき
			}
		}
	}
...
func main() {
	W, N, K := scan.Int(), scan.Int(), scan.Int()
	ABs := make([][]int, N, N)
	for i := 0; i < N; i++ {
		ABs[i] = []int{scan.Int(), scan.Int()}
	}
	fmt.Println(Solve(ABs, N, W, K))
}
