//go:generate echo "https://atcoder.jp/contests/abc051/tasks/abc051_c"
package main
...
func main() {
	sx, sy, tx, ty := scan.Int(), scan.Int(), scan.Int(), scan.Int()
	dx, dy := tx-sx, ty-sy
	p := ""
	// 1回目
	p += strings.Repeat("R", dx)
	p += strings.Repeat("U", dy)
	p += strings.Repeat("L", dx)
	p += strings.Repeat("D", dy)
	// 2回目
	p += "D"
