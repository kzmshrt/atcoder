//go:generate echo "https://atcoder.jp/contests/abc062/tasks/arc074_a"
package main
...
func main() {
	// １つの分割を決めたら、のこりの２つはなるべく縦または横の長さを均等にするのが最適である
	...
	ans := imin(H, W)
	for i := 1; i < H; i++ {
		chmin(&ans, imax(i*(W%2), imax(iabs(i*((W+1)/2)-(H-i)*W), iabs(i*(W/2)-(H-i)*W))))
	}
	for i := 1; i < W; i++ {
		chmin(&ans, imax(i*(H%2), imax(iabs(i*((H+1)/2)-(W-i)*H), iabs(i*(H/2)-(W-i)*H))))
	}
	fmt.Println(ans)
}
