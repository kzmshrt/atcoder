//go:generate echo "https://atcoder.jp/contests/abc067/tasks/arc078_a"
package main
...
func solve2() {
	...
	sum[0] = in.Int()
	for i := 1; i < N; i++ {
		sum[i] = sum[i-1] + in.Int()
	}
	min := math.MaxInt32
	for _, sumv := range sum[:N-1] {
		chmin(&min, iabs(sum[N-1]-2*sumv))
	}
	fmt.Println(min)
}
...
func solve1() {
	...
	tempSum := 0
	for _, av := range a[:N-1] {
		tempSum += av
		chmin(&minDiff, iabs(tempSum-(sum-tempSum)))
	}
	fmt.Println(minDiff)
}
