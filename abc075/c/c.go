//go:generate echo "https://atcoder.jp/contests/abc075/tasks/abc075_c"
package main
...
func dfs(v, N int, graph [][]bool, visited []bool) {
	visited[v] = true
	for v2 := 0; v2 < N; v2++ {
		if !graph[v][v2] {
			continue
...
func main() {
	...
	limit := 50
	as, bs := make([]int, limit, limit), make([]int, limit, limit)
	graph, visited := make([][]bool, limit, limit), make([]bool, limit, limit)
	for i := range graph {
		graph[i] = make([]bool, limit, limit)
	}
	for i := 0; i < M; i++ {
		as[i], bs[i] = scan.Int()-1, scan.Int()-1
		graph[as[i]][bs[i]], graph[bs[i]][as[i]] = true, true
	}
	ans := 0
	for i := 0; i < M; i++ {
		graph[as[i]][bs[i]], graph[bs[i]][as[i]] = false, false
		for j := 0; j < N; j++ {
			visited[j] = false
		}
