//go:generate echo "https://atcoder.jp/contests/abc091/tasks/arc092_a"
package main
...
func main() {
	N := scan.Int()
	abs, cds := make([][]int, N, N), make([][]int, N, N)
	for i := 0; i < N; i++ {
		abs[i] = scan.Ints(2)
	}
	for i := 0; i < N; i++ {
	...
	sort.Slice(abs, func(i, j int) bool { return abs[i][1] > abs[j][1] })
	sort.Slice(cds, func(i, j int) bool { return cds[i][0] < cds[j][0] })
