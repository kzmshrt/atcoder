//go:generate echo "https://atcoder.jp/contests/abc092/tasks/arc093_a"
package main
...
func main() {
	...
	// 	As[i] = scan.Int()
	// }
	// costs := make([][]int, N+2, N+2)
	// for i := range costs {
	// 	costs[i] = make([]int, N+2, N+2)
	// }
	// for i := 0; i < N+1; i++ {
	// 	costs[i][i+1] = iabs(As[i] - As[i+1])
	// 	if i < N {
	// 		costs[i][i+2] = iabs(As[i] - As[i+2])
	// 	}
	// }
	// for i := 1; i <= N; i++ {
	// 	c := 0
