//go:generate echo "https://atcoder.jp/contests/abc095/tasks/arc096_a"
package main
...
func main() {
	A, B, C, X, Y := scan.Int(), scan.Int(), scan.Int(), scan.Int(), scan.Int()
	if A+B < 2*C {
		fmt.Println(A*X + B*Y)
		return
	}
	if A > 2*C && B > 2*C {
		fmt.Println(2 * C * imax(X, Y))
	} else if A > 2*C {
		fmt.Println(2*C*X + imax(0, Y-X)*B)
	} else if B > 2*C {
		fmt.Println(2*C*Y + imax(0, X-Y)*A)
	} else {
		fmt.Println(2*C*imin(X, Y) + imax(0, Y-X)*B + imax(0, X-Y)*A)
	}
}
