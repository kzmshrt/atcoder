//go:generate echo "https://atcoder.jp/contests/abc104/tasks/abc104_c"
package main
...
func solve(D, G int, ps, cs []int) int {
	min := math.MaxInt64
	for mask := 0; mask < (1 << D); mask++ {
		score, curMin, restMax := 0, 0, -1
		for i := 0; i < D; i++ {
			if mask>>i&1 == 1 {
		...
		if score < G {
			restMaxScore := 100 * (restMax + 1)
			need := (G - score + restMaxScore - 1) / restMaxScore
			if need >= ps[restMax] {
				continue
			}
			curMin += need
...
func main() {
	D, G := scan.Int(), scan.Int()
	ps, cs := make([]int, D, D), make([]int, D, D)
	for i := 0; i < D; i++ {
		ps[i], cs[i] = scan.Int(), scan.Int()
	}
	fmt.Println(solve(D, G, ps, cs))
}
