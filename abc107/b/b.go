//go:generate echo "https://atcoder.jp/contests/abc107/tasks/abc107_b"
package main
...
func solve(H, W int, field []string) string {
	rskip, cskip := map[int]bool{}, map[int]bool{}
	for i := 0; i < imax(H, W); i++ {
		a, b := true, true
		for j := 0; j < imax(H, W); j++ {
			if i < H && j < W && field[i][j] != '.' {
				a = false
			}
			if i < W && j < H && field[j][i] != '.' {
				b = false
			}
		}
		if i < H && a {
...
func main() {
	H, W := scan.Int(), scan.Int()
	field := scan.Strings(H)
	fmt.Print(solve(H, W, field))
}
