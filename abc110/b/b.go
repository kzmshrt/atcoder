//go:generate echo "https://atcoder.jp/contests/abc110/tasks/abc110_b"
package main
...
func solve(X, Y int, xs, ys []int) bool {
	xmax, ymin := xs[0], ys[0]
	for _, x := range xs[1:] {
		chmax(&xmax, x)
	}
	...
	return xmax < ymin && X < ymin && xmax < Y
}

func main() {
	N, M, X, Y := scan.Int(), scan.Int(), scan.Int(), scan.Int()
	xs, ys := scan.Ints(N), scan.Ints(M)
	if solve(X, Y, xs, ys) {
		fmt.Println("No War")
	} else {
		fmt.Println("War")
