//go:generate echo "https://atcoder.jp/contests/abc110/tasks/abc110_c"
package main
...
func solve(S, T string) bool {
	possible := true
	ma, ima := map[uint8]uint8{}, map[uint8]uint8{}
	for i := 0; i < len(S); i++ {
		if _, ok := ma[S[i]]; ok && ma[S[i]] != T[i] {
			possible = false
		}
		if _, ok := ima[T[i]]; ok && ima[T[i]] != S[i] {
			possible = false
		}
		ma[S[i]] = T[i]
		ima[T[i]] = S[i]
