//go:generate echo "https://atcoder.jp/contests/abc112/tasks/abc112_c"
package main
...
func solve(xs, ys, hs []int) (int, int, int) {
	for cx := 0; cx <= 100; cx++ {
		for cy := 0; cy <= 100; cy++ {
			...
			for i := 0; i < len(xs); i++ {
				if hs[i] != 0 {
					H = hs[i] + iabs(xs[i]-cx) + iabs(ys[i]-cy)
					break
				}
			}
			if H == 0 {
			...
			for i := 0; i < len(xs); i++ {
				v := iabs(xs[i]-cx) + iabs(ys[i]-cy)
				if (hs[i] == 0 && H-v > 0) || (hs[i] > 0 && hs[i]+v != H) {
					f = false
					break
				}
			}
...
func main() {
	N := scan.Int()
	xs, ys, hs := make([]int, N, N), make([]int, N, N), make([]int, N, N)
	for i := 0; i < N; i++ {
		xs[i], ys[i], hs[i] = scan.Int(), scan.Int(), scan.Int()
	}
