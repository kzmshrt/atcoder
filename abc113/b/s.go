//go:generate echo "https://atcoder.jp/contests/abc113/tasks/abc113_b"
package main
...
func solve(Hs []float64, T, A float64) int {
	...
	mini, min := -1, math.MaxFloat64
	for i, H := range Hs {
		if v := fabs((T - H*0.006) - A); v < min {
			min = v
			mini = i
		}
	}
...
func main() {
	N, T, A := scan.Int(), scan.Float64(), scan.Float64()
	Hs := scan.Float64s(N)
	fmt.Println(solve(Hs, T, A))
}
