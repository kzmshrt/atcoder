//go:generate echo "https://atcoder.jp/contests/abc132/tasks/abc132_b"
package main
...
func solve(ps []int) int {
	...
	for i := 1; i < len(ps)-1; i++ {
		if sort.IsSorted(sort.IntSlice(ps[i-1:i+2])) || sort.IsSorted(sort.Reverse(sort.IntSlice(ps[i-1:i+2]))) {
			c++
		}
	}
	return c
