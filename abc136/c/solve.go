//go:generate echo "https://atcoder.jp/contests/abc136/tasks/abc136_c"
package main
...
func solve() {
	...
	// 単調非減少にしたいから最初の要素は必ず -1 したほうがいい
	Hs[0]--
	for i := 1; i < N-1; i++ {
		if Hs[i] > Hs[i+1] { // 今が次より大きい
			Hs[i]--
			if Hs[i] > Hs[i+1] || Hs[i-1] > Hs[i] {
				fmt.Println("No")
				return
			}
		} else if Hs[i-1] <= Hs[i]-1 { // 前が今-1以下 → 単調費減少にしたいから予め -1 しておく
			Hs[i]--
		}
	}
	fmt.Println("Yes")
...
// 後方から見て、最小値を保持して、Hs[i]-1 > 最小値を維持できるかどうかを確認する
func solve2() {
	N := scan.Int()
	Hs := scan.Ints(N)
