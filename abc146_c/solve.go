//go:generate echo "https://atcoder.jp/contests/abc146/tasks/abc146_c"
package main
...
func Solve(A, B, X int) int {
	for d := 10; 0 < d; d-- {
		p := ipow(10, d-1)
		if v := X - A*p - B*d; 0 <= v {
			return imin(p+v/A, ipow(10, 9))
		}
	}
	return 0
}

func Solve2(A, B, X int) int {
	l, r := 0, 1000000001
	for r-l > 1 {
		m := (l + r) / 2
		if A*m+B*len(strconv.Itoa(m)) > X {
			r = m
		} else {
			l = m
		}
...
func main() {
	A, B, X := scan.Int(), scan.Int(), scan.Int()
	fmt.Println(Solve2(A, B, X))
}
