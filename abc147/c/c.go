//go:generate echo "https://atcoder.jp/contests/abc147/tasks/abc147_c"
package main
...
func main() {
	N := scan.Int()
	testimony := make([]map[int]bool, N, N)
	for i := range testimony {
		testimony[i] = map[int]bool{}
		A := scan.Int()
		for j := 0; j < A; j++ {
			testimony[i][scan.Int()-1] = scan.Int() == 1
		}
	}
	max := 0
