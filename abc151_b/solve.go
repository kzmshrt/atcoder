//go:generate echo "https://atcoder.jp/contests/abc151/tasks/abc151_b"
package main
...
func Solve(N, K, M int, As []int) int {
	if v := N*M - isum(As); v <= K {
		return imax(v, 0)
	}
	return -1
}
...
func main() {
	N, K, M := scan.Int(), scan.Int(), scan.Int()
	As := scan.Ints(N - 1)
	fmt.Println(Solve(N, K, M, As))
}
