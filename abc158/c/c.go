//go:generate echo "https://atcoder.jp/contests/abc158/tasks/abc158_c"
package main
...
func main() {
	A, B := scan.Float64(), scan.Float64()
	amin, amax := int(math.Ceil(A/0.08)), int(math.Floor((A+1)/0.08))-1
	bmin, bmax := int(math.Ceil(B/0.1)), int(math.Floor((B+1)/0.1))-1
	if amax < bmin || bmax < amin {
		fmt.Println(-1)
	} else {
		fmt.Println(imax(amin, bmin))
	}
}
