//go:generate echo "https://atcoder.jp/contests/abc165/tasks/abc165_c"
package main
...
func main() {
	N, M, Q = scan.Int(), scan.Int(), scan.Int()
	as, bs, cs, ds = make([]int, Q, Q), make([]int, Q, Q), make([]int, Q, Q), make([]int, Q, Q)
	for i := 0; i < Q; i++ {
		as[i], bs[i], cs[i], ds[i] = scan.Int(), scan.Int(), scan.Int(), scan.Int()
	}
	dfs([]int{1, 1})
	fmt.Println(ans)
}
