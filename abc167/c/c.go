//go:generate echo "https://atcoder.jp/contests/abc167/tasks/abc167_c"
package main
...
func main() {
	N, M, X := scan.Int(), scan.Int(), scan.Int()
	Cs, As := make([]int, N, N), make([][]int, N, N)
	for i := 0; i < N; i++ {
		Cs[i], As[i] = scan.Int(), scan.Ints(M)
	}
	cost, possible := math.MaxInt32, false
