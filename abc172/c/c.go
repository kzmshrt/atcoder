//go:generate echo "https://atcoder.jp/contests/abc172/tasks/abc172_c"
package main
...
func main() {
	N, M, K := scan.Int(), scan.Int(), scan.Int()
	As, Bs := make([]int, N+1, N+1), make([]int, M+1, M+1)
	// 累積和を求めながら入力を取得する
	for i := 1; i <= N; i++ {
		As[i] = As[i-1] + scan.Int()
	}
	for i := 1; i <= M; i++ {
		Bs[i] = Bs[i-1] + scan.Int()
	}
	// B から全部読むところから考え始める
	ans, j := 0, M
	for i := 0; i <= N; i++ {
