//go:generate echo "https://atcoder.jp/contests/abc180/tasks/abc180_b"
package main
...
func EuclideanDistance(X []int) float64 {
	d := 0
	for _, x := range X {
		d += ipow(x, 2)
	}
...
func ChebyshevDistance(X []int) int {
	d := 0
	for _, x := range X {
		chmax(&d, iabs(x))
	}
...
func main() {
	N := scan.Int()
	xs := scan.Ints(N)
	fmt.Println(ManhattanDistance(xs))
	fmt.Println(EuclideanDistance(xs))
	fmt.Println(ChebyshevDistance(xs))
}
