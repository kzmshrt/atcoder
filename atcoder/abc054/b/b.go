//go:generate echo "https://atcoder.jp/contests/abc054/tasks/abc054_b"
package main
...
func main() {
	...
	As, Bs := make([][]rune, N, N), make([][]rune, M, M)
	for i := 0; i < N; i++ {
		As[i] = []rune(scan.String())
	}
	for i := 0; i < M; i++ {
		Bs[i] = []rune(scan.String())
	}
	for i := 0; i <= N-M; i++ {
		for j := 0; j <= N-M; j++ {
			c := true
			for ay, by := i, 0; ay < i+M; ay, by = ay+1, by+1 {
				if string(As[ay][j:j+M]) != string(Bs[by]) {
					c = false
					break
				}
			}
