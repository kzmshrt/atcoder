//go:generate echo "https://atcoder.jp/contests/abc057/tasks/abc057_b"
package main
...
func main() {
	...
	as, bs, cs, ds := make([]int, N, N), make([]int, N, N), make([]int, M, M), make([]int, M, M)
	for i := 0; i < N; i++ {
		as[i], bs[i] = scan.Int(), scan.Int()
	}
	for i := 0; i < M; i++ {
		cs[i], ds[i] = scan.Int(), scan.Int()
	}
	for i := 0; i < N; i++ {
		min, minj := math.MaxInt64, -1
		for j := 0; j < M; j++ {
			if v := iabs(cs[j]-as[i]) + iabs(ds[j]-bs[i]); v < min {
				min, minj = v, j
			}
		}
		fmt.Println(minj + 1)
