//go:generate echo "https://atcoder.jp/contests/abc084/tasks/abc084_c"
package main
...
func main() {
	N := scan.Int()
	C, S, F := make([]int, N, N), make([]int, N, N), make([]int, N, N)
	for i := 0; i < N-1; i++ {
		C[i], S[i], F[i] = scan.Int(), scan.Int(), scan.Int()
	}
	C[N-1], S[N-1], F[N-1] = 0, 0, 1
	for i := 0; i < N; i++ {
		t := 0
		for j := i; j < N; j++ {
			if t <= S[j] {
