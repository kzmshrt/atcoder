//go:generate echo "https://atcoder.jp/contests/abc085/tasks/abc085_c"
package main
...
func main() {
	N, Y := scan.Int(), scan.Int()
	for i := 0; i <= N && 10000*i <= Y; i++ {
		for j := 0; i+j <= N && 10000*i+5000*j <= Y; j++ {
			if k := N - i - j; 10000*i+5000*j+1000*k == Y {
				fmt.Println(i, j, k)
				return
			}
		}
