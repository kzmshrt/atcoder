//go:generate echo "https://atcoder.jp/contests/abc107/tasks/arc101_a"
package main
...
func solve(xs []int, K int) int {
	min := math.MaxInt64
	for i := 0; i < len(xs)-K+1; i++ {
		chmin(&min, xs[i+K-1]-xs[i]+imin(iabs(xs[i]), iabs(xs[i+K-1])))
	}
	return min
}
