//go:generate echo "https://atcoder.jp/contests/abc119/tasks/abc119_c"
package main
...
func solve(N, A, B, C int, ls []int, n, a, b, c int) int {
	if n == N {
		if a == 0 || b == 0 || c == 0 {
			return math.MaxInt32
		}
		return iabs(a-A) + iabs(b-B) + iabs(c-C)
	}

	r := solve(N, A, B, C, ls, n+1, a, b, c)
	...
	chmin(&r, solve(N, A, B, C, ls, n+1, a+ls[n], b, c)+ca)
	chmin(&r, solve(N, A, B, C, ls, n+1, a, b+ls[n], c)+cb)
	chmin(&r, solve(N, A, B, C, ls, n+1, a, b, c+ls[n])+cc)

	return r
}

func main() {
	N, A, B, C := scan.Int(), scan.Int(), scan.Int(), scan.Int()
	ls := scan.Ints(N)
	fmt.Println(solve(N, A, B, C, ls, 0, 0, 0, 0))
}
