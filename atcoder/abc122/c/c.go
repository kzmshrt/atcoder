//go:generate echo "https://atcoder.jp/contests/abc122/tasks/abc122_c"
package main
...
func solve(S string, LR [][]int) []int {
	sum := make([]int, len(S), len(S))
	for i := 1; i < len(S); i++ {
		sum[i] += sum[i-1]
		if S[i-1:i+1] == "AC" {
			sum[i]++
	...
	ans := make([]int, len(LR), len(LR))
	for i, lr := range LR {
		ans[i] = sum[lr[1]] - sum[lr[0]]
	}
	return ans
}

func main() {
	_, Q := scan.Int(), scan.Int()
	S := scan.String()
	LR := make([][]int, Q, Q)
	for i := range LR {
		LR[i] = []int{scan.Int() - 1, scan.Int() - 1}
	}
	for _, a := range solve(S, LR) {
		fmt.Println(a)
	}
}
