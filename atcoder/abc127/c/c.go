//go:generate echo "https://atcoder.jp/contests/abc127/tasks/abc127_c"
package main
...
func solve(N int, LRs [][]int) int {
	lmax, rmin := 0, N+1
	for _, LR := range LRs {
		chmax(&lmax, LR[0])
		chmin(&rmin, LR[1])
	}
	return imax(0, rmin-lmax+1)
}
...
func main() {
	...
	LRs := make([][]int, M, M)
	for i := range LRs {
		LRs[i] = []int{scan.Int(), scan.Int()}
	}
	fmt.Println(solve(N, LRs))
}
