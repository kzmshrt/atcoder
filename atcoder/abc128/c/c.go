//go:generate echo "https://atcoder.jp/contests/abc128/tasks/abc128_c"
package main
...
func solve(N, M int, masks []uint64, ps []int) int {
	c := 0
	for i := 0; i < ipow(2, N); i++ {
		all := true
		for j := 0; j < M; j++ {
			if bits.OnesCount64(uint64(i)&masks[j])%2 != ps[j] {
				all = false
				continue
			}
		}
...
func main() {
	...
	masks := make([]uint64, M, M)
	for i := range masks {
		k := scan.Int()
		for j := 0; j < k; j++ {
			masks[i] |= 1 << (scan.Int() - 1)
		}
	}
	ps := scan.Ints(M)
	fmt.Println(solve(N, M, masks, ps))
}
