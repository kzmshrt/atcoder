//go:generate echo "https://atcoder.jp/contests/abc133/tasks/abc133_b"
package main
...
func solve(Xs [][]int) int {
	...
	for i := 0; i < len(Xs)-1; i++ {
		for j := i + 1; j < len(Xs); j++ {
			s := 0.0
			for d := 0; d < len(Xs[0]); d++ {
				s += math.Pow(float64(Xs[i][d]-Xs[j][d]), 2)
			}
			if v := math.Sqrt(s); v == math.Trunc(v) {
				p++
			}
		}
	}
