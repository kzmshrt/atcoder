//go:generate echo "https://atcoder.jp/contests/abc177/tasks/abc177_c"
package main
...
func solve(As []int) int {
	setMintMod(1000000007)
	sum := newMint(0)
	for i := 0; i < len(As)-1; i++ {
		for j := i + 1; j < len(As); j++ {
			x, y := newMint(As[i]), newMint(As[j])
			sum = sum.Add(x.Mul(y))
		}
	}
	return int(sum)
...
func solve2(As []mint) mint {
	as := newMint(0) // all sum
	for _, A := range As {
		as = as.Add(A)
	}
	s := newMint(0) // sum
	for _, A := range As {
		s = s.Add(A.Mul(as.Sub(A)))
	}
	return s.Div(newMint(2))
}
...
func (s *scanner) Mint() mint {
	n, _ := strconv.Atoi(s.String())
	return newMint(n)
}

func (s *scanner) Mints(l int) []mint {
	if l == 0 {
		return []mint{}
	}
	sl := make([]mint, l, l)
