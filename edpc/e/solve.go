//go:generate echo "https://atcoder.jp/contests/dp/tasks/dp_e"
package main
...
func main() {
	...
	for i := 0; i < N; i++ {
		weights[i], values[i] = scan.Int(), scan.Int()
	}

	t := make([][]int, 110, 110)
	for i := range t {
		t[i] = make([]int, 100100, 100100)
		for j := range t[i] {
			t[i][j] = math.MaxInt32
		}
	}
	...
	for i := 0; i < N; i++ {
		for sumv := 0; sumv < 100100; sumv++ {
			// i番目の品物を選ぶ場合
			if sumv-values[i] >= 0 {
				chmin(&t[i+1][sumv], t[i][sumv-values[i]]+weights[i])
			}
			// i番目の品物を選ばない場合
			chmin(&t[i+1][sumv], t[i][sumv])
		}
	}
