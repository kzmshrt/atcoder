//go:generate echo "https://atcoder.jp/contests/tdpc/tasks/tdpc_contest"
package main
...
// 解けなかった
// func Solve(scoreAllocs []int) int {
// 	n := len(scoreAllocs)
//
// 	maxScoreSum := 0
// 	for _, score := range scoreAllocs {
// 		maxScoreSum += score
// 	}
//
// 	t := make([][]bool, n+1, n+1)
// 	for i := range t {
// 		t[i] = make([]bool, maxScoreSum+1, maxScoreSum+1)
// 	}
//
// 	for i, score := range scoreAllocs {
//
// 	}
//
// 	patternSum := 0
// 	for _, patternExists := range t[n-1] {
// 		if patternExists {
// 			patternSum++
// 		}
// 	}
...
func Solve(scores []int) int {
	...
	t[0][0] = true            // ０問解いたときに０点でありえる
	for i := 1; i <= n; i++ { // i問目について考える
		for j, possible := range t[i-1] {
			if possible { // i-1問目までの解く・解かないのパターンで得られる合計得点の集合を抽出
				t[i][j] = true             // 今の問題を解かない場合、そのままの点数を次の集合へ伝播
				t[i][j+scores[i-1]] = true // 今の問題を解く場合、今の問題の点数を加算して次の集合へ伝播
			}
		}
	}
...
func main() {
	N := scan.Int()
	scoreAllocs := scan.Ints(N)
	fmt.Println(Solve(scoreAllocs))
}
