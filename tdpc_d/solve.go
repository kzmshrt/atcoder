//go:generate echo "https://atcoder.jp/contests/tdpc/tasks/tdpc_dice"
package main
...
// 		for j := 0; j <= 6; j++ {
// 			for k := 0; k <= D; k++ {
// 				kj := newMint(k).Mul(newMint(j))
// 				t[i][j][kj] = t[i][j][kj].Add(t[i-1][j][k])
// 			}
// 		}
// 	}
//
...
// 		for j := range t[N-1][i] {
// 			if j == 0 {
// 				dividable = dividable.Add(t[N-1][i][j])
// 			}
// 			sum = sum.Add(t[N-1][i][j])
// 		}
// 	}
//
// 	return float64(dividable) / float64(sum)
// }

func Solve(N, D int) float64 {
	p2 := []int{0, 1, 0, 2, 0, 1} // prime 2
	p3 := []int{0, 0, 1, 0, 0, 1} // prime 3
	p5 := []int{0, 0, 0, 0, 1, 0} // prime 5
	...
	t := make([][][]float64, cnt[0]+1, cnt[0]+1)
	for i := range t {
		t[i] = make([][]float64, cnt[1]+1, cnt[1]+1)
		for j := range t[i] {
			t[i][j] = make([]float64, cnt[2]+1, cnt[2]+1)
		}
	}

	t[0][0][0] = 1
	for lp := 0; lp < N; lp++ {
		next := make([][][]float64, cnt[0]+1, cnt[0]+1)
		for i := range next {
			next[i] = make([][]float64, cnt[1]+1, cnt[1]+1)
			for j := range next[i] {
				next[i][j] = make([]float64, cnt[2]+1, cnt[2]+1)
			}
		}
		...
		for x := 0; x <= cnt[0]; x++ {
			for y := 0; y <= cnt[1]; y++ {
				for z := 0; z <= cnt[2]; z++ {
					for i := 0; i < 6; i++ {
						nx := imin(cnt[0], x+p2[i])
						ny := imin(cnt[1], y+p3[i])
						nz := imin(cnt[2], z+p5[i])
						next[nx][ny][nz] += t[x][y][z] / 6
					}
				}
			}
		}
	...
	return t[cnt[0]][cnt[1]][cnt[2]]
}

func main() {
	N, D := scan.Int(), scan.Int()
	fmt.Printf("%.9f\n", Solve(N, D))
}
